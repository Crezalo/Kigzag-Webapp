  ...
  try {
    ...
    if (username != "")
      new_User = await pool.query(
        "UPDATE Users SET UserName=$1 WHERE UserAddress=$2",
        [username, address]
      );
    if (iscreator != null)
      new_User = await pool.query(
        "UPDATE Users SET IsCreator=$1 WHERE UserAddress=$2",
        [iscreator, address]
      );
    if (twitterhandle != "")
      new_User = await pool.query(
        "UPDATE Users SET TwitterHandle=$1 WHERE UserAddress=$2",
        [twitterhandle, address]
      );
    if (discord != "")
      new_User = await pool.query(
        "UPDATE Users SET Discord=$1 WHERE UserAddress=$2",
        [discord, address]
      );
    if (tiktok != "")
      new_User = await pool.query(
        "UPDATE Users SET Tiktok=$1 WHERE UserAddress=$2",
        [tiktok, address]
      );
    if (instagram != "")
      new_User = await pool.query(
        "UPDATE Users SET Instagram=$1 WHERE UserAddress=$2",
        [instagram, address]
      );
    if (youtube != "")
      new_User = await pool.query(
        "UPDATE Users SET Youtube=$1 WHERE UserAddress=$2",
        [youtube, address]
      );
    if (website != "")
      new_User = await pool.query(
        "UPDATE Users SET Website=$1 WHERE UserAddress=$2",
        [website, address]
      );
  ...
  try {
    ...
    if (valid) {
      const new_token = await pool.query(
        "INSERT INTO Creator_token (TokenAddress) VALUES ($1) RETURNING*;",
        [address]
      );
