...
import {
  addCreatorColabData,
  getCreatorColabData,
  updateCreatorColabData,
} from "../services/api-services/creator/colab_api";
...
interface ColabTabProp {
  creator: string;
  onCreatorProfile: boolean;
}
const ColabTab = ({ creator, onCreatorProfile }: ColabTabProp) => {
  ...
  const GetRequests = () => {
    useEffect(() => {
      async function getData() {
        if (onCreatorProfile) {
          const result = await getUserColabData();
          ...
          const result = await getUserColabAllCreatorData();
          // setUserShoutoutData(result);
          console.log(result);
          for (var i = 0; i < result.length; i++) {
            if (result[i].platform == 0) setReqCountInsta(reqCountInsta + 1);
  ...
  const addNewPlatform = async () => {
    const result = await addCreatorColabData(
      addUpdatePlatform,
      addUpdateCPW,
      addUpdatePrice
    );
  ...
  const updatePlatform = async () => {
    const result = await updateCreatorColabData(
      scenario,
      addUpdateCPW,
      addUpdatePrice
    );
    ...
    >
      <ThemeProvider theme={toolTipTheme}>
        ...
        {count == 0 && onCreatorProfile ? (
          <div style={{marginTop:"15vh"}}>{creator} doesn't support collabs.</div>
        ) : (
          <>
